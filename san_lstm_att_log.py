...
class Answer_Generator():
    ...
    def build_model(self):
        ...
        return loss, image, question, label, #accuracy
...
def get_train_val_data():
    ...
    with h5py.File(input_img_h5,'r') as hf:
        ...
        train_img_feature = np.array(tem)
        tem = hf.get('images_test')
        test_img_feature = np.array(tem)
    # load h5 file
    ...
    if img_norm:
        ...
        #test_img_feature = np.divide(test_img_feature, np.transpose(np.tile(tem,(4096,1))))
        test_img_feature = np.transpose(test_img_feature,(0,2,3,1))
        test_img_feature = np.divide(test_img_feature, np.transpose(np.tile(tem,[512,1,1,1]),(1,2,3,0)) + 1e-8)

    return dataset, train_img_feature, train_data, test_img_feature, test_data
...
def train():
    ...
    model = Answer_Generator(
            ...
            dim_image = train_img_feature[0].shape,
    ...
    #tf_loss, tf_image, tf_question, tf_label, tf_acc = model.build_model()
    tf_loss, tf_image, tf_question, tf_label = model.build_model()
    #g_tf_answer, g_tf_image, g_tf_question, g_tf_prob_att1, g_tf_prob_att2 = model.build_generator()
    tf.summary.scalar('loss', tf_loss)
    #tf.summary.scalar('acc', tf_acc)
    ...
    for itr in range(max_itr):
        ...
        current_img = train_img_feature[current_img_list,:]

        # do the training process!!!
        #_, loss, acc, summary = sess.run(
        _, loss, summary = sess.run(
                #[train_op, tf_loss, tf_acc, merged],
